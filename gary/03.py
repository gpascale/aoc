...
def getcoord(grid, neighbor):
  i, j = neighbor
  while j > 0 and grid[i][j-1] in '0123456789':
    j -= 1
  nlen = neighbor[1] - j + 1
  while j+nlen < len(grid[i]) and grid[i][j+nlen] in '0123456789':
    nlen += 1
  return (i, j, nlen)

def getcoords(coords, grid, neighbors):
  digits = set(map(str, range(10)))
  for neighbor in neighbors:
    if grid[neighbor[0]][neighbor[1]] in digits:
      coords.add(getcoord(grid, neighbor))


def main():
  grid = make_2d_list(data)
  total = 0
  #print(neighbors(grid, 1, 73))
  #coords = set()
  #getcoords(coords, grid, neighbors(grid, 1, 73))
  #exit()
  for i in range(len(grid)):
    for j in range(len(grid[i])):
      if grid[i][j] not in '0123456789.':
        coords = set()
        getcoords(coords, grid, neighbors(grid, i, j))
        prod = 1
        if len(coords) > 1:
          for coord in coords:
            ii, jj, nlen = coord
            num = int(''.join(grid[ii][jj:jj+nlen]))
            prod *= num
          total += prod
  print(total)
